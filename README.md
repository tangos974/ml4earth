...
You can visualize them yourself as we've included those of our best performing model's training runs in the releases tab. To visualize them, place them in the root folder of the project and run the following command:
...
To run iferrence yourself, there is also [Jupyter notebook](model_load_and_inferring.ipynb) that can be used to run inference on a local machine. Just dowload the model from the releases tab, place it in the root folder of the project, and run the notebook.
